...
def get_datamodule(args, mode):
    ...
    if mode == "transfer":
        if args.dataset != "ImageNet":
            return TransferCIFARDataModule(args)
    elif mode == "discover":
        if args.dataset == "ImageNet":
...
class TransferCIFARDataModule(pl.LightningDataModule):
    def __init__(self, args):
        super().__init__()
        self.data_dir = args.data_dir
        self.download = args.download
    ...
    def setup(self, stage=None):
        ...
        # for n in unlabeled_classes: 
        #     index = torch.where(targets == n)[0]
        #     indices.append(index)
        # indices = torch.cat(indices, dim=0)
        # train_indices_lab = indices.view(self.num_unlabeled_classes, -1).transpose(0,1).flatten()

        # train_indices_lab = np.where(
        #     np.isin(np.array(self.train_dataset.targets), unlabeled_classes)
        # )[0]
        # self.train_dataset = torch.utils.data.Subset(self.train_dataset, train_indices_lab)
        ###
