...
from .utils import TransformTwice, TransformKtimes, RandomTranslateWithReflect, TwoStreamBatchSampler, Solarization, GaussianBlur
# from .concat import ConcatDataset
import torchvision.transforms as transforms

class CIFAR10_GCD(data.Dataset):
    ...
    def __init__(self, root, split='train+test',
        ...
        sort_id = sort_id[sort_tgt<self.num_lab_classes]
        sort_id = np.reshape(sort_id, (-1, 2))
        idx = sort_id[:,0]
        self.mask = np.zeros_like(self.targets)
        self.mask[idx] = 1

        # np.savetxt('mask.txt', self.mask, fmt='%d')
        # np.savetxt('idx.txt', idx, fmt='%05d')
        # np.savetxt('targets.txt', np.array(self.targets)[idx], fmt='%02d')
...
def CIFAR10Data_GCD(root, split='train', aug=None, target_list=range(10)):
    ...
    if aug==None:
        ...
        transform2 = transforms.Compose([
            ...
            flip_and_color_jitter,
            GaussianBlur(0.1),
            Solarization(0.2), # different from transfo2
            normalize,
        ])
        transform = TransformTwice(transform1, transform2)
    dataset = CIFAR10_GCD(root=root, split=split, transform=transform, target_list=target_list)
    return dataset

def CIFAR10LoaderMixGCD(root, batch_size, split='train',num_workers=2, aug=None, shuffle=True, sampler=None, target_list=range(10)):
    dataset = CIFAR10Data_GCD(root, split, aug, target_list)
...
class CIFAR100_GCD(CIFAR10_GCD):
...
def CIFAR100Data_GCD(root, split='train', aug=None, target_list=range(100)):
    global_crops_scale = (0.4, 1.)
    flip_and_color_jitter = transforms.Compose([
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomApply(
    ...
    if aug==None:
        ...
        transform = TransformTwice(transform1, transform2)
    dataset = CIFAR100_GCD(root=root, split=split, transform=transform, target_list=target_list)
    return dataset

def CIFAR100LoaderMixGCD(root, batch_size, split='train',num_workers=2, aug=None, shuffle=True, sampler=None, target_list=range(100)):
    dataset = CIFAR100Data_GCD(root, split, aug, target_list)
